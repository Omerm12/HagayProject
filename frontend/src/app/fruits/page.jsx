...
export default function FruitsPage() {
  ...
  useEffect(() => {
    const load = async () => {
      setLoading(true);
      const data = await fetchProductsByCategory("פירות");
      setProducts(data);
      setLoading(false);
    };
    load();
  ...
  return (
    <div className="min-h-screen bg-[#F6E8CA] px-4 py-10 font-georgia flex flex-col items-center">
      <div className="text-center mt-20 max-w-3xl">
        <h1 className="text-7xl font-extrabold text-orange-700 mb-4">פירות</h1>
        <p className="text-2xl font-extrabold text-gray-800 leading-relaxed whitespace-pre-line">
          פירות איכותיים וטעימים היישר מהחקלאים הכי טובים, במשלוח מהיר עד פתח הדלת.{"\n"}
          * על כל משלוח מעל 200 ש"ח מתנה
        </p>
      </div>
